//Nico Ramos
//csci159 lab2 part 2 


#include <iostream>
#include <string>
#include <iomanip>
using std::cout;
using std::endl;
using std::cin;
using std::string;
using std::setprecision;
using std::setw;
using std::fixed;
using std::left;
using std::right;

int main()
{
	//Displaying the welcome message
	
	cout << "Welcome to EvelKall monitoring!" << endl;
	cout << "We estimate your staff's working hours based on the result of weekly call sample. " <<endl;
	cout << "At  the moment assume you have 3 staff members, and round the result to the nearest 0.1 hours. " << endl;

	//Displaying the names of each three employees 
	
	string employee1, employee2, employee3;
	cout <<"Please enter the first name of the first employee: " << endl;
	cin >> employee1;
	cout << employee1 << endl;
	cout <<"Please enter the second name of the second employee: " << endl;
	cin >> employee2;
	cout << employee2 << endl;
	cout <<"Please enter the third name of the third employee: " << endl;
	cin >> employee3;
	cout << employee3 << endl;
	
	//Displaying how many time the employee answered a call
	
	int numCalls1, numCalls2, numCalls3;

	cout << "Enter how many calls: " << employee1 << " answered: " << numCalls1 << endl;
	cin >> numCalls1;
	
	cout << "Enter how many calls: " << employee2 << " answered: " << numCalls2 << endl;
	cin >> numCalls2;

	cout << "Enter how many calls: " << employee3 << " answered: " << numCalls3 << endl;
	cin >> numCalls3;

	//Displaying how many went are unanswered 

	int missedCall;
	
	cout << "Enter how many did: " << employee1 << " answer? " << numCalls1 << endl;
	cin >> numCalls1;

	cout << "Enter how many did: " << employee2 << " answser? " << numCalls2 << endl;
	cin >> numCalls2;

	cout << "Enter how many did: " << employee3 << " answer? " << numCalls3 <<endl;
	cin >> numCalls3;
	
	cout << "How many calls went unanswered? " << endl;
	cin >> missedCall;

	//Displaying the total numbes made of the week 
	
	double  totalCalls = numCalls1 + numCalls2 + numCalls3 + missedCall;
	double hours1 = 168 * ( numCalls1 / totalCalls);
	double hours2 = 168 * ( numCalls2 / totalCalls);
	double hours3 = 168 * ( numCalls3 / totalCalls);
	double hoursUnanswered = 168 * ( missedCall / totalCalls );
	
	cout << "Based on the information you provided, here is the following results: " << endl;
	cout << endl;
	cout << "Estimated weekly hours: " << endl; 
	cout << fixed << setprecision(1);
	cout << setw(12) << left << employee1 << setw(10) << right << hours1 << endl;
	cout << setw(12) << left << employee2 << setw(10) << right << hours2 << endl;
	cout << setw(12) << left << employee3 << setw(10) << right << hours3 << endl;
	cout << setw(12) << left << "Unansered" << setw(10) << right << hoursUnanswered << endl;
	cout << endl;
	cout << "Thanks for using EvelKall Monitoring!" << endl;
	
	
	return 0;
	
}
